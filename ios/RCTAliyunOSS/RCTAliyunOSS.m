...
- (NSArray<NSString *> *)supportedEvents {
    return @[@"uploadProgress", @"downloadProgress", @"getBuckerFiles", @"resumableUploadProgress",@"checkObjectExist",@"resumableUploadSuccess",@"resumableUploadFail"];
}
...
RCT_EXPORT_METHOD(initAliOSSWithKey:(NSString *)AccessKey
...
RCT_REMAP_METHOD(getBucketFiles,bucketName:(NSString *)BucketName
                 file:(NSString *)file
                 maxkeys:(int)maxkeys
    ...
    [getBucketTask continueWithBlock:^id(OSSTask *task) {
        if (!task.error) {
            ...
            for (NSDictionary * objectInfo in result.contents) {
                ...
                NSLog(@"Key = %@",[objectInfo valueForKey:@"Key"]);
...
//                                                             @"buckerFiles":array}];
            NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
            [dict setObject:[NSNumber numberWithBool:result.isTruncated] forKey:@"isTruncated"];
            [dict setObject:nextMarker forKey:@"nextMarker"];
            [dict setObject:array forKey:@"bucketFiles"];
            resolve(dict);
        } else {
            reject(@"-1", @"error", task.error);
        }
        return nil;
    }];
}
...
RCT_REMAP_METHOD(checkObjectExist, bucketName:(NSString *)BucketName
    ...
    if (exist) {
        ...
        reject(@"-1", [NSNumber numberWithBool:NO], nil);
    }
...
//异步断点上传
RCT_REMAP_METHOD(resumableUpload, bucketName:(NSString *)BucketName
    ...
    [[[[[[OSSTask taskWithResult:nil] continueWithBlock:^id(OSSTask *task) {
      ...
      continueWithSuccessBlock:^id(OSSTask *task) {
          ...
          resumableUpload.uploadProgress = ^(int64_t bytesSent, int64_t totalBytesSent, int64_t totalBytesExpectedToSend) {
              ...
              [self sendEventWithName: @"resumableUploadProgress" body:@{@"currentSize": [NSString stringWithFormat:@"%lld",totalBytesSent],
     ...
     continueWithBlock:^id(OSSTask *task) {
         if (task.error) {
             ...
             NSLog(@"上传完成!");
             [self sendEventWithName: @"resumableUploadProgress" body:nil];
             resolve(@"onSuccess");
